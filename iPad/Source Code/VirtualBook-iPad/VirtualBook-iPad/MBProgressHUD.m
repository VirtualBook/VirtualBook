...
+ (MB_INSTANCETYPE)showHUDIfNeedAddedTo:(UIView *)view animated:(BOOL)animated
{
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated title:(NSString *) title
{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.labelText = title;
...
+ (MB_INSTANCETYPE)showHUDOrChangeTitleAddedTo:(UIView *)view animated:(BOOL)animated title:(NSString *) title
{
    NSArray *huds = [MBProgressHUD allHUDsForView:view];
    if (huds.count == 0)
    {
    ...
    {
        // Change titile
        MBProgressHUD *hub = [huds firstObject];
        
        hub.labelText = title;
        
        return MB_AUTORELEASE(hub);
    }
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		if (mode == MBProgressHUDModeAnnularDeterminate)
        {
            ...
            if (_progressColorForCircleLoader)
            {
                [(MBRoundProgressView *)indicator setProgressTintColor:_progressColorForCircleLoader];
            }
            //
		}
	} 
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_isNeedSquare)
    {
        CGFloat denta = boxRect.size.width - boxRect.size.height;
        
        boxRect.size.height = boxRect.size.width;
        boxRect.origin.y = boxRect.origin.y-  denta / 2;
    }
